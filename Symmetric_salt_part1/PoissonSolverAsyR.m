...
for ik=1:floor(length(len)/2)+1
    ...
    if ik == floor(length(len)/2)+1
        varphi_old(1,index_fdm(ik,1:len(1,ik))) = linspace(0, zeta(1,end), length(x_1));
    else
        varphi_old(1,index_fdm(ik,1:len(1,ik))) = linspace(0, zeta(1,n-ik+1), length(x_1));
    end
    while errPB > err_tol
        N = len(1,ik);
        a = zeros(1,N);
        ...
        if ik==floor(length(len)/2)+1
            d(N,1) = 2*zeta(1,end)+ d(N,1);
            d(1,1) = 2*0+ d(1,1);
        else
            d(N,1) = 2*zeta(1,n-ik+1)+ d(N,1);
            d(1,1) = 2*0+ d(1,1);
        end
